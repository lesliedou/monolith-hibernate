...
public class FillRuleItemResourceIT {
     ...
     */
    public static FillRuleItem createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static FillRuleItem createUpdatedEntity(EntityManager em) {
        FillRuleItem fillRuleItem = new FillRuleItem()
    ...
    void getAllFillRuleItems() throws Exception {
        ...
        fillRuleItemRepository.saveAndFlush(fillRuleItem);
    ...
    void getAllFillRuleItemsWithEagerRelationshipsIsEnabled() throws Exception {
        when(fillRuleItemServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFillRuleItemMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(fillRuleItemServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllFillRuleItemsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        verify(fillRuleItemRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void getAllFillRuleItemsByFillRuleIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, SysFillRule.class).isEmpty()) {
            fillRuleItemRepository.saveAndFlush(fillRuleItem);
            fillRule = SysFillRuleResourceIT.createEntity(em);
        } else {
            // fillRule = TestUtil.findAll(em, SysFillRule.class).get(0);
    ...
    void putExistingFillRuleItem() throws Exception {
        ...
        // Disconnect from session so that the updates on updatedFillRuleItem are not directly saved in db
        em.detach(updatedFillRuleItem);
        updatedFillRuleItem
            .sortValue(UPDATED_SORT_VALUE)
            .fieldParamType(UPDATED_FIELD_PARAM_TYPE)
            .fieldParamValue(UPDATED_FIELD_PARAM_VALUE)
