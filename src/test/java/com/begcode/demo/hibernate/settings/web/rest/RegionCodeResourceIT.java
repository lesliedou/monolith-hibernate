...
public class RegionCodeResourceIT {
     ...
     */
    public static RegionCode createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static RegionCode createUpdatedEntity(EntityManager em) {
        RegionCode regionCode = new RegionCode()
            .name(UPDATED_NAME)
    ...
    void getAllRegionCodes() throws Exception {
        ...
        regionCodeRepository.saveAndFlush(regionCode);
    ...
    void getAllRegionCodesWithEagerRelationshipsIsEnabled() throws Exception {
        when(regionCodeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRegionCodeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(regionCodeServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllRegionCodesWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        verify(regionCodeRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void getAllRegionCodesByChildrenIsEqualToSomething() throws Exception {
        RegionCode children;
        if (TestUtil.findAll(em, RegionCode.class).isEmpty()) {
            regionCodeRepository.saveAndFlush(regionCode);
            children = RegionCodeResourceIT.createEntity(em);
        } else {
            // children = TestUtil.findAll(em, RegionCode.class).get(0);
    ...
    void getAllRegionCodesByParentIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, RegionCode.class).isEmpty()) {
            regionCodeRepository.saveAndFlush(regionCode);
            parent = RegionCodeResourceIT.createEntity(em);
        } else {
            // parent = TestUtil.findAll(em, RegionCode.class).get(0);
