...
public class SysFillRuleResourceIT {
     ...
     */
    public static SysFillRule createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static SysFillRule createUpdatedEntity(EntityManager em) {
        SysFillRule sysFillRule = new SysFillRule()
            .name(UPDATED_NAME)
            .code(UPDATED_CODE)
            .desc(UPDATED_DESC)
    ...
    void getAllSysFillRules() throws Exception {
        ...
        sysFillRuleRepository.saveAndFlush(sysFillRule);
    ...
    void getAllSysFillRulesByRuleItemsIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, FillRuleItem.class).isEmpty()) {
            sysFillRuleRepository.saveAndFlush(sysFillRule);
            ruleItems = FillRuleItemResourceIT.createEntity(em);
        } else {
            // ruleItems = TestUtil.findAll(em, FillRuleItem.class).get(0);
            ruleItems = new FillRuleItem(); // todo
        }
        em.persist(ruleItems);
        em.flush();
