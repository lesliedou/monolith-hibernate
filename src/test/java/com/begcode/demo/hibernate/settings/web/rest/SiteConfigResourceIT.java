...
public class SiteConfigResourceIT {
     ...
     */
    public static SiteConfig createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static SiteConfig createUpdatedEntity(EntityManager em) {
        SiteConfig siteConfig = new SiteConfig()
    ...
    void getAllSiteConfigs() throws Exception {
        ...
        siteConfigRepository.saveAndFlush(siteConfig);
    ...
    void getAllSiteConfigsByItemsIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, CommonFieldData.class).isEmpty()) {
            siteConfigRepository.saveAndFlush(siteConfig);
            items = CommonFieldDataResourceIT.createEntity(em);
        } else {
            // items = TestUtil.findAll(em, CommonFieldData.class).get(0);
    ...
    void partialUpdateSiteConfigWithPatch() throws Exception {
        ...
        partialUpdatedSiteConfig.categoryKey(UPDATED_CATEGORY_KEY).disabled(UPDATED_DISABLED).builtIn(UPDATED_BUILT_IN);
