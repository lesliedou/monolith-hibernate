...
public class AuthorityResourceIT {
    ...
    void getAllAuthoritiesWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        verify(authorityRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void getAllAuthoritiesByChildrenIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Authority.class).isEmpty()) {
            authorityRepository.saveAndFlush(authority);
            children = AuthorityResourceIT.createEntity(em);
        } else {
            // children = TestUtil.findAll(em, Authority.class).get(0);
    ...
    void getAllAuthoritiesByViewPermissionsIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ViewPermission.class).isEmpty()) {
            authorityRepository.saveAndFlush(authority);
            viewPermissions = ViewPermissionResourceIT.createEntity(em);
        } else {
            // viewPermissions = TestUtil.findAll(em, ViewPermission.class).get(0);
    ...
    void getAllAuthoritiesByApiPermissionsIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ApiPermission.class).isEmpty()) {
            authorityRepository.saveAndFlush(authority);
            apiPermissions = ApiPermissionResourceIT.createEntity(em);
        } else {
            // apiPermissions = TestUtil.findAll(em, ApiPermission.class).get(0);
    ...
    void getAllAuthoritiesByParentIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Authority.class).isEmpty()) {
            authorityRepository.saveAndFlush(authority);
            parent = AuthorityResourceIT.createEntity(em);
        } else {
            // parent = TestUtil.findAll(em, Authority.class).get(0);
