...
public class ApiPermissionResourceIT {
    ...
    void getAllApiPermissionsWithEagerRelationshipsIsEnabled() throws Exception {
        when(apiPermissionServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restApiPermissionMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(apiPermissionServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllApiPermissionsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        verify(apiPermissionRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void getAllApiPermissionsByChildrenIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ApiPermission.class).isEmpty()) {
            apiPermissionRepository.saveAndFlush(apiPermission);
            children = ApiPermissionResourceIT.createEntity(em);
        } else {
            // children = TestUtil.findAll(em, ApiPermission.class).get(0);
    ...
    void getAllApiPermissionsByParentIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ApiPermission.class).isEmpty()) {
            apiPermissionRepository.saveAndFlush(apiPermission);
            parent = ApiPermissionResourceIT.createEntity(em);
        } else {
            // parent = TestUtil.findAll(em, ApiPermission.class).get(0);
    ...
    void getAllApiPermissionsByAuthoritiesIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Authority.class).isEmpty()) {
            apiPermissionRepository.saveAndFlush(apiPermission);
            authorities = AuthorityResourceIT.createEntity(em);
        } else {
            // authorities = TestUtil.findAll(em, Authority.class).get(0);
    ...
    void partialUpdateApiPermissionWithPatch() throws Exception {
        ...
        partialUpdatedApiPermission.code(UPDATED_CODE).method(UPDATED_METHOD).url(UPDATED_URL).status(UPDATED_STATUS);
