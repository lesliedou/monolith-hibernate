...
public class ResourceCategoryResourceIT {
    ...
    void getAllResourceCategoriesWithEagerRelationshipsIsEnabled() throws Exception {
        when(resourceCategoryServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restResourceCategoryMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(resourceCategoryServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllResourceCategoriesWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        verify(resourceCategoryRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void getAllResourceCategoriesByFilesIsEqualToSomething() throws Exception {
        UploadFile files;
        if (TestUtil.findAll(em, UploadFile.class).isEmpty()) {
            resourceCategoryRepository.saveAndFlush(resourceCategory);
            files = UploadFileResourceIT.createEntity(em);
        } else {
            // files = TestUtil.findAll(em, UploadFile.class).get(0);
    ...
    void getAllResourceCategoriesByChildrenIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ResourceCategory.class).isEmpty()) {
            resourceCategoryRepository.saveAndFlush(resourceCategory);
            children = ResourceCategoryResourceIT.createEntity(em);
        } else {
            // children = TestUtil.findAll(em, ResourceCategory.class).get(0);
    ...
    void getAllResourceCategoriesByImagesIsEqualToSomething() throws Exception {
        UploadImage images;
        if (TestUtil.findAll(em, UploadImage.class).isEmpty()) {
            resourceCategoryRepository.saveAndFlush(resourceCategory);
            images = UploadImageResourceIT.createEntity(em);
        } else {
            // images = TestUtil.findAll(em, UploadImage.class).get(0);
    ...
    void getAllResourceCategoriesByParentIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ResourceCategory.class).isEmpty()) {
            resourceCategoryRepository.saveAndFlush(resourceCategory);
            parent = ResourceCategoryResourceIT.createEntity(em);
        } else {
            // parent = TestUtil.findAll(em, ResourceCategory.class).get(0);
