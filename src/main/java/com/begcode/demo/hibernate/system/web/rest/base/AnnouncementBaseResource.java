...
public class AnnouncementBaseResource {
    ...
    public ResponseEntity<PageRecord<AnnouncementDTO>> getAllAnnouncements(
        ...
        page = announcementQueryService.findByCriteria(criteria, pageable);
    ...
    public void exportToExcel(HttpServletResponse response) {
        List<AnnouncementDTO> data = announcementService.findAll(Pageable.unpaged()).getContent();
    ...
    public ResponseEntity<List<AnnouncementDTO>> getUnReadAnnouncements(@PathVariable AnnoCategory category, Pageable page) {
        ...
        Page<AnnouncementDTO> byCriteria;
        ...
        if (!ids.isEmpty()) {
            ...
            byCriteria = announcementQueryService.findByCriteria(announcementCriteria, page);
        } else {
            byCriteria = new PageImpl<>(new ArrayList<>(), page, 0);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), byCriteria);
        return ResponseEntity.ok().headers(headers).body(byCriteria.getContent());
    }
