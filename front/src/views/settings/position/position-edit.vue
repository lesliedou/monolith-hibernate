...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const position = ref<IPosition>(new Position());
    position.value = Object.assign(position.value, props.baseData);
    if (positionId.value) {
      const data = await apiService.settings.positionService.find(Number(positionId.value));
      if (data) {
        position.value = Object.assign(position.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            position.value = Object.assign(position.value, result);
            saveOrUpdateApi(position.value)
              .then(res => {
                position.value = Object.assign(position.value, res);
                positionId.value = position.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/settings/position/' + positionId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(position.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (positionId.value ? '编辑岗位' : '新增岗位') : null,
                  bordered: false,
                  size: 'small',
                }}
