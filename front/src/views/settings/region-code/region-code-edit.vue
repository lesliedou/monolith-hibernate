...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const regionCode = ref<IRegionCode>(new RegionCode());
    regionCode.value = Object.assign(regionCode.value, props.baseData);
    if (regionCodeId.value) {
      const data = await apiService.settings.regionCodeService.find(Number(regionCodeId.value));
      if (data) {
        regionCode.value = Object.assign(regionCode.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            regionCode.value = Object.assign(regionCode.value, result);
            saveOrUpdateApi(regionCode.value)
              .then(res => {
                regionCode.value = Object.assign(regionCode.value, res);
                regionCodeId.value = regionCode.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/settings/region-code/' + regionCodeId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(regionCode.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (regionCodeId.value ? '编辑行政区划码' : '新增行政区划码') : null,
                  bordered: false,
                  size: 'small',
                }}
