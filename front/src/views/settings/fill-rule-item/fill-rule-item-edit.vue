...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const fillRuleItem = ref<IFillRuleItem>(new FillRuleItem());
    fillRuleItem.value = Object.assign(fillRuleItem.value, props.baseData);
    if (fillRuleItemId.value) {
      const data = await apiService.settings.fillRuleItemService.find(Number(fillRuleItemId.value));
      if (data) {
        fillRuleItem.value = Object.assign(fillRuleItem.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            fillRuleItem.value = Object.assign(fillRuleItem.value, result);
            saveOrUpdateApi(fillRuleItem.value)
              .then(res => {
                fillRuleItem.value = Object.assign(fillRuleItem.value, res);
                fillRuleItemId.value = fillRuleItem.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/settings/fill-rule-item/' + fillRuleItemId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(fillRuleItem.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (fillRuleItemId.value ? '编辑填充规则条目' : '新增填充规则条目') : null,
                  bordered: false,
                  size: 'small',
                }}
