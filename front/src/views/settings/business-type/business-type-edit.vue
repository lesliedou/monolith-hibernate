...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const businessType = ref<IBusinessType>(new BusinessType());
    businessType.value = Object.assign(businessType.value, props.baseData);
    if (businessTypeId.value) {
      const data = await apiService.settings.businessTypeService.find(Number(businessTypeId.value));
      if (data) {
        businessType.value = Object.assign(businessType.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            businessType.value = Object.assign(businessType.value, result);
            saveOrUpdateApi(businessType.value)
              .then(res => {
                businessType.value = Object.assign(businessType.value, res);
                businessTypeId.value = businessType.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/settings/business-type/' + businessTypeId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(businessType.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (businessTypeId.value ? '编辑业务类型' : '新增业务类型') : null,
                  bordered: false,
                  size: 'small',
                }}
