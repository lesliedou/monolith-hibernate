...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const siteConfig = ref<ISiteConfig>(new SiteConfig());
    siteConfig.value = Object.assign(siteConfig.value, props.baseData);
    if (siteConfigId.value) {
      const data = await apiService.settings.siteConfigService.find(Number(siteConfigId.value));
      if (data) {
        siteConfig.value = Object.assign(siteConfig.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            siteConfig.value = Object.assign(siteConfig.value, result);
            saveOrUpdateApi(siteConfig.value)
              .then(res => {
                siteConfig.value = Object.assign(siteConfig.value, res);
                siteConfigId.value = siteConfig.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/settings/site-config/' + siteConfigId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(siteConfig.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (siteConfigId.value ? '编辑网站配置' : '新增网站配置') : null,
                  bordered: false,
                  size: 'small',
                }}
