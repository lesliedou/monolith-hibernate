...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const sysLog = ref<ISysLog>(new SysLog());
    sysLog.value = Object.assign(sysLog.value, props.baseData);
    if (sysLogId.value) {
      const data = await apiService.log.sysLogService.find(Number(sysLogId.value));
      if (data) {
        sysLog.value = Object.assign(sysLog.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            sysLog.value = Object.assign(sysLog.value, result);
            saveOrUpdateApi(sysLog.value)
              .then(res => {
                sysLog.value = Object.assign(sysLog.value, res);
                sysLogId.value = sysLog.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/log/sys-log/' + sysLogId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(sysLog.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (sysLogId.value ? '编辑系统日志' : '新增系统日志') : null,
                  bordered: false,
                  size: 'small',
                }}
