...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const taskJobConfig = ref<ITaskJobConfig>(new TaskJobConfig());
    taskJobConfig.value = Object.assign(taskJobConfig.value, props.baseData);
    if (taskJobConfigId.value) {
      const data = await apiService.taskjob.taskJobConfigService.find(Number(taskJobConfigId.value));
      if (data) {
        taskJobConfig.value = Object.assign(taskJobConfig.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            taskJobConfig.value = Object.assign(taskJobConfig.value, result);
            saveOrUpdateApi(taskJobConfig.value)
              .then(res => {
                taskJobConfig.value = Object.assign(taskJobConfig.value, res);
                taskJobConfigId.value = taskJobConfig.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/taskjob/task-job-config/' + taskJobConfigId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(taskJobConfig.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (taskJobConfigId.value ? '编辑定时任务' : '新增定时任务') : null,
                  bordered: false,
                  size: 'small',
                }}
