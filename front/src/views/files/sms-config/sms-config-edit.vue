...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const smsConfig = ref<ISmsConfig>(new SmsConfig());
    smsConfig.value = Object.assign(smsConfig.value, props.baseData);
    if (smsConfigId.value) {
      const data = await apiService.files.smsConfigService.find(Number(smsConfigId.value));
      if (data) {
        smsConfig.value = Object.assign(smsConfig.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            smsConfig.value = Object.assign(smsConfig.value, result);
            saveOrUpdateApi(smsConfig.value)
              .then(res => {
                smsConfig.value = Object.assign(smsConfig.value, res);
                smsConfigId.value = smsConfig.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/sms-config/' + smsConfigId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(smsConfig.value);
                }
              })
              .catch(error => {
    ...
    const formGroup = computed(() => [
      {
        title: props.containerType === 'router' ? '短信配置' : null,
        operation: [],
        component: {
          name: 'a-form',
          props: {
            modelName: 'BASE_ENTITY',
            model: smsConfig.value,
            labelWidth: '120px',
            fieldMapToTime: [],
            compact: true,
            alwaysShowLines: 1,
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (smsConfigId.value ? '编辑短信配置' : '新增短信配置') : null,
                  bordered: false,
                  size: 'small',
                }}
