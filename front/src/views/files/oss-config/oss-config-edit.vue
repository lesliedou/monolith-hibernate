...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const ossConfig = ref<IOssConfig>(new OssConfig());
    ossConfig.value = Object.assign(ossConfig.value, props.baseData);
    if (ossConfigId.value) {
      const data = await apiService.files.ossConfigService.find(Number(ossConfigId.value));
      if (data) {
        ossConfig.value = Object.assign(ossConfig.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            ossConfig.value = Object.assign(ossConfig.value, result);
            saveOrUpdateApi(ossConfig.value)
              .then(res => {
                ossConfig.value = Object.assign(ossConfig.value, res);
                ossConfigId.value = ossConfig.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/oss/oss-config/' + ossConfigId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(ossConfig.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (ossConfigId.value ? '编辑对象存储配置' : '新增对象存储配置') : null,
                  bordered: false,
                  size: 'small',
                }}
