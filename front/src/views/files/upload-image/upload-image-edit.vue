...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const uploadImage = ref<IUploadImage>(new UploadImage());
    uploadImage.value = Object.assign(uploadImage.value, props.baseData);
    if (uploadImageId.value) {
      const data = await apiService.files.uploadImageService.find(Number(uploadImageId.value));
      if (data) {
        uploadImage.value = Object.assign(uploadImage.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            uploadImage.value = Object.assign(uploadImage.value, result);
            saveOrUpdateApi(uploadImage.value)
              .then(res => {
                uploadImage.value = Object.assign(uploadImage.value, res);
                uploadImageId.value = uploadImage.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/files/upload-image/' + uploadImageId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(uploadImage.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (uploadImageId.value ? '编辑上传图片' : '新增上传图片') : null,
                  bordered: false,
                  size: 'small',
                }}
