...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const apiPermission = ref<IApiPermission>(new ApiPermission());
    apiPermission.value = Object.assign(apiPermission.value, props.baseData);
    if (apiPermissionId.value) {
      const data = await apiService.system.apiPermissionService.find(Number(apiPermissionId.value));
      if (data) {
        apiPermission.value = Object.assign(apiPermission.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            apiPermission.value = Object.assign(apiPermission.value, result);
            saveOrUpdateApi(apiPermission.value)
              .then(res => {
                apiPermission.value = Object.assign(apiPermission.value, res);
                apiPermissionId.value = apiPermission.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/api-permission/' + apiPermissionId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(apiPermission.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (apiPermissionId.value ? '编辑API权限' : '新增API权限') : null,
                  bordered: false,
                  size: 'small',
                }}
