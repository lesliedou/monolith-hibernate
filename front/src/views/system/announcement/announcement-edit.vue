...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const announcement = ref<IAnnouncement>(new Announcement());
    announcement.value = Object.assign(announcement.value, props.baseData);
    if (announcementId.value) {
      const data = await apiService.system.announcementService.find(Number(announcementId.value));
      if (data) {
        announcement.value = Object.assign(announcement.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            announcement.value = Object.assign(announcement.value, result);
            saveOrUpdateApi(announcement.value)
              .then(res => {
                announcement.value = Object.assign(announcement.value, res);
                announcementId.value = announcement.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/announcement/' + announcementId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(announcement.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (announcementId.value ? '编辑系统通告' : '新增系统通告') : null,
                  bordered: false,
                  size: 'small',
                }}
