...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const smsMessage = ref<ISmsMessage>(new SmsMessage());
    smsMessage.value = Object.assign(smsMessage.value, props.baseData);
    if (smsMessageId.value) {
      const data = await apiService.system.smsMessageService.find(Number(smsMessageId.value));
      if (data) {
        smsMessage.value = Object.assign(smsMessage.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            smsMessage.value = Object.assign(smsMessage.value, result);
            saveOrUpdateApi(smsMessage.value)
              .then(res => {
                smsMessage.value = Object.assign(smsMessage.value, res);
                smsMessageId.value = smsMessage.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/sms-message/' + smsMessageId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(smsMessage.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (smsMessageId.value ? '编辑短信消息' : '新增短信消息') : null,
                  bordered: false,
                  size: 'small',
                }}
