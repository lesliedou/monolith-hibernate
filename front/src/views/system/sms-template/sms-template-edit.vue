...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const smsTemplate = ref<ISmsTemplate>(new SmsTemplate());
    smsTemplate.value = Object.assign(smsTemplate.value, props.baseData);
    if (smsTemplateId.value) {
      const data = await apiService.system.smsTemplateService.find(Number(smsTemplateId.value));
      if (data) {
        smsTemplate.value = Object.assign(smsTemplate.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            smsTemplate.value = Object.assign(smsTemplate.value, result);
            saveOrUpdateApi(smsTemplate.value)
              .then(res => {
                smsTemplate.value = Object.assign(smsTemplate.value, res);
                smsTemplateId.value = smsTemplate.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/sms-template/' + smsTemplateId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(smsTemplate.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (smsTemplateId.value ? '编辑消息模板' : '新增消息模板') : null,
                  bordered: false,
                  size: 'small',
                }}
