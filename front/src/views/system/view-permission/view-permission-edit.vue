...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const viewPermission = ref<IViewPermission>(new ViewPermission());
    viewPermission.value = Object.assign(viewPermission.value, props.baseData);
    if (viewPermissionId.value) {
      const data = await apiService.system.viewPermissionService.find(Number(viewPermissionId.value));
      if (data) {
        viewPermission.value = Object.assign(viewPermission.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            viewPermission.value = Object.assign(viewPermission.value, result);
            saveOrUpdateApi(viewPermission.value)
              .then(res => {
                viewPermission.value = Object.assign(viewPermission.value, res);
                viewPermissionId.value = viewPermission.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/view-permission/' + viewPermissionId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(viewPermission.value);
                }
              })
              .catch(error => {
    ...
    const pageConfig = reactive<any>({
      ...
      operations: [
        ...
        {
          hide: () => {
            return !!viewPermission.value.id;
          },
          type: 'primary',
          theme: 'save',
          click: saveOrUpdate,
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (viewPermissionId.value ? '编辑可视权限' : '新增可视权限') : null,
                  bordered: false,
                  size: 'small',
                }}
