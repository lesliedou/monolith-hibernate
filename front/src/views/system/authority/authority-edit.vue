...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const authority = ref<IAuthority>(new Authority());
    authority.value = Object.assign(authority.value, props.baseData);
    if (authorityId.value) {
      const data = await apiService.system.authorityService.find(Number(authorityId.value));
      if (data) {
        authority.value = Object.assign(authority.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            authority.value = Object.assign(authority.value, result);
            saveOrUpdateApi(authority.value)
              .then(res => {
                authority.value = Object.assign(authority.value, res);
                authorityId.value = authority.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/authority/' + authorityId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(authority.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (authorityId.value ? '编辑角色' : '新增角色') : null,
                  bordered: false,
                  size: 'small',
                }}
