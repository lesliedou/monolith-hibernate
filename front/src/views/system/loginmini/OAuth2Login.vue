  ...
  userStore.ThirdLogin(params).then(res => {
    if (res && res.userInfo) {
      notification.success({
        message: '登录成功',
        description: `欢迎回来: ${res.userInfo.realname}`,
        duration: 3,
      });
    } else {
      notification.error({
        message: 'Translation missing for sys.login.errorTip',
        description: ((res.response || {}).data || {}).message || res.message || 'Translation missing for sys.login.networkExceptionMsg',
        duration: 4,
      });
    }
  });
