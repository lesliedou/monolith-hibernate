...
export default defineComponent({
  ...
  async setup(props) {
    ...
    user.value = Object.assign(user.value, props.baseData);
    if (userId.value) {
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            user.value = Object.assign(user.value, result);
            saveOrUpdateApi(user.value)
              .then(res => {
                user.value = Object.assign(user.value, res);
                userId.value = user.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/user/' + userId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(user.value);
                }
              })
              .catch(error => {
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (userId.value ? '编辑用户' : '新增用户') : null,
                  bordered: false,
                  size: 'small',
                }}
