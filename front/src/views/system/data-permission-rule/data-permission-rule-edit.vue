...
export default defineComponent({
  ...
  async setup(props) {
    ...
    const dataPermissionRule = ref<IDataPermissionRule>(new DataPermissionRule());
    dataPermissionRule.value = Object.assign(dataPermissionRule.value, props.baseData);
    if (dataPermissionRuleId.value) {
      ...
      if (data) {
        dataPermissionRule.value = Object.assign(dataPermissionRule.value, data);
      }
    }
    ...
    const saveOrUpdate = () => {
      validate()
        .then(result => {
          if (result) {
            dataPermissionRule.value = Object.assign(dataPermissionRule.value, result);
            saveOrUpdateApi(dataPermissionRule.value)
              .then(res => {
                dataPermissionRule.value = Object.assign(dataPermissionRule.value, res);
                dataPermissionRuleId.value = dataPermissionRule.value.id + '';
                message.success(submitButtonTitlePrefix + '成功！');
                if (props.containerType === 'router') {
                  const { fullPath } = route; //获取当前路径
                  tabStore.closeTabByKey(fullPath, router).then(() => {
                    go('/system/data-permission-rule/' + dataPermissionRuleId.value + '/edit', true);
                  });
                } else {
                  if (!props.savedOpen) {
                    ctx?.$emit('refresh', { update: true, containerType: props.containerType });
                  }
                  (ctx?.$refs['BASE_ENTITY'] as any).setFieldsValue(dataPermissionRule.value);
                }
              })
              .catch(error => {
    ...
    const pageConfig = reactive<any>({
      ...
      operations: [
        ...
        {
          hide: () => {
            return !!dataPermissionRule.value.id;
          },
          type: 'primary',
          theme: 'save',
          click: saveOrUpdate,
    ...
    const formGroup = computed(() => [
      {
        title: props.containerType === 'router' ? '数据权限规则' : null,
        operation: [],
        component: {
          name: 'a-form',
          props: {
            modelName: 'BASE_ENTITY',
            model: dataPermissionRule.value,
            labelWidth: '120px',
            fieldMapToTime: [],
            compact: true,
            alwaysShowLines: 1,
            ...
            <div>
              <a-card
                {...{
                  ...
                  title: props.containerType === 'router' ? (dataPermissionRuleId.value ? '编辑数据权限规则' : '新增数据权限规则') : null,
                  bordered: false,
                  size: 'small',
                }}
